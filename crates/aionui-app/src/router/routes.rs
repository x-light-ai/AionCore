...
use crate::xaiwork::auth::mask_upstream_login_routes;
...
pub fn create_router_with_all_state(services: &AppServices, states: ModuleStates, ws_state: WsHandlerState) -> Router {
    ...
    .merge(office_proxy)
    .merge(public_assets)
    // FORK-CUSTOM: upstream account/login routes stay registered upstream but
    // are unreachable in the XAIWork runtime.
    .layer(middleware::from_fn(mask_upstream_login_routes))
